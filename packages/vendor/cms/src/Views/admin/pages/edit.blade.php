...
<div class="dashboard-content">
    ...
    <div class="page-header">
        <div>
            ...
            <p class="page-description">Modifiez les paramètres de votre page</p>
        </div>
        ...
        <div class="page-actions">
            ...
            <button class="btn btn-success" id="saveAndPublishBtn" style="display: {{ $page->status === 'published' ? 'none' : 'inline-flex' }}">
                <i class="fas fa-globe me-2"></i>Sauvegarder et publier
            </button>
            @if($page->status === 'published')
                <button class="btn btn-warning" id="unpublishBtn">
    ...
    <div class="main-card-modern">
        <form id="pageForm" method="POST" action="{{ route('cms.admin.pages.update', ['etablissementId' => $etablissement->id, 'id' => $page->id]) }}">
            @csrf
            @method('PUT')
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" class="form-control @error('slug') is-invalid @enderror" 
                                           id="slug" name="slug" value="{{ old('slug', $page->slug) }}" placeholder="titre-de-la-page">
                                </div>
                        ...
                        <div class="card-body-modern">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('visibility') is-invalid @enderror" name="visibility" id="visibility">
                                    ...
                                    <option value="private" {{ old('visibility', $page->visibility) == 'private' ? 'selected' : '' }}>Privé (nécessite connexion)</option>
                                    <option value="password" {{ old('visibility', $page->visibility) == 'password' ? 'selected' : '' }}>Protégé par mot de passe</option>
                                </select>
                                @error('visibility')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="mb-3" id="passwordField" style="display: {{ old('visibility', $page->visibility) == 'password' ? 'block' : 'none' }};">
                                <label for="password" class="form-label">Mot de passe</label>
                                <input type="password" class="form-control @error('password') is-invalid @enderror" 
                                       name="password" id="password" value="{{ old('password', $page->password) }}">
                            ...
                            <div class="form-check mb-3">
                                <input type="checkbox" class="form-check-input" id="is_home" name="is_home" value="1" {{ old('is_home', $page->is_home) ? 'checked' : '' }}>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="datetime-local" class="form-control @error('published_at') is-invalid @enderror" 
                                       name="published_at" id="published_at" value="{{ old('published_at', $page->published_at ? $page->published_at->format('Y-m-d\TH:i') : '') }}">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="seo_title" name="meta[seo_title]" 
                                               value="{{ old('meta.seo_title', $page->getMeta('seo_title')) }}" placeholder="{{ $page->title }}">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="seo_keywords" name="meta[seo_keywords]" 
                                               value="{{ old('meta.seo_keywords', $page->getMeta('seo_keywords')) }}" placeholder="mot-clé1, mot-clé2, mot-clé3">
                                ...
                                <div class="col-12">
                                    <div class="mb-3">
                                        ...
                                        <textarea class="form-control" id="seo_description" name="meta[seo_description]" 
                                                  rows="3" placeholder="Description pour les moteurs de recherche">{{ old('meta.seo_description', $page->getMeta('seo_description')) }}</textarea>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="input-group">
                                            <input type="text" class="form-control" id="og_image" name="meta[og_image]" 
                                                   value="{{ old('meta.og_image', $page->getMeta('og_image')) }}" placeholder="URL de l'image">
            ...
            <div class="row">
                <div class="col-12">
                    <div class="card-modern mb-4">
                        <div class="card-header-modern">
                            <h3><i class="fas fa-chart-line me-2"></i>Informations</h3>
                        </div>
                        <div class="card-body-modern">
                            <div class="row">
                                <div class="col-md-3">
                                ...
                                <div class="col-md-3">
                                    <div class="info-item">
                                        ...
                                        <div class="info-value">{{ $page->created_at->format('d/m/Y H:i') }}</div>
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="info-item">
                                        <div class="info-label">Dernière modification</div>
                                        <div class="info-value">{{ $page->updated_at->format('d/m/Y H:i') }}</div>
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="info-item">
                                        <div class="info-label">Auteur</div>
                                        <div class="info-value">{{ $page->user->name ?? 'Inconnu' }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body text-center py-4">
                ...
                <h4>Supprimer la page</h4>
                <p class="text-muted">Êtes-vous sûr de vouloir supprimer la page "{{ $page->title }}" ?<br>Cette action est irréversible.</p>
                <div class="mt-4">
...
const pagesListUrl = @json(route('cms.admin.dashboard', ['etablissementId' => $etablissement->id, 'slug' => ($etablissement->slug ?: \Illuminate\Support\Str::slug((string) $etablissement->name))]) . '?section=pages');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    titleInput.addEventListener('blur', function() {
        if (!slugInput.value || slugInput.value === '{{ $page->slug }}') {
    ...
    function unpublishPage() {
        Swal.fire({
            ...
            text: 'La page ne sera plus visible publiquement.',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                fetch(`/admin/cms/${etablissementId}/pages/${pageId}/unpublish`, {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                        'Accept': 'application/json',
                ...
                .then(data => {
                    hideLoading();
                    if (data.success) {
                        Swal.fire('Dépubliée!', data.message, 'success').then(() => {
                            location.reload();
                        });
                    } else {
    ...
    function deletePage() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                fetch(`/admin/cms/${etablissementId}/pages/${pageId}`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                        'Accept': 'application/json'
                ...
                .then(data => {
                    hideLoading();
                    if (data.success) {
                        Swal.fire('Supprimée!', data.message, 'success').then(() => {
                            window.location.href = pagesListUrl;
                        });
                    } else {
                        Swal.fire('Erreur!', data.message, 'error');
                    }
