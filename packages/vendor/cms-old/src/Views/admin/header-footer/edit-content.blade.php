...
<body>
    <div
        ...
        data-blocks-url="{{ route('cms.admin.blocks.api', ['etablissementId' => $etablissement->id]) }}">
    ...
    <script>
        ...
        async function loadCmsBlocks() {
            ...
            list.innerHTML = '<div class="hf-block-state">Chargement des blocs...</div>';
            setEditorStatus('Chargement des blocs...');

            try {
                const response = await fetch(shell.dataset.blocksUrl, { headers: { Accept: 'application/json' } });
                const payload = await response.json();
                if (!payload.success) throw new Error(payload.message || 'Chargement des blocs impossible');

                cmsBlocks = (payload.blocks || [])
                    .filter(block => isHeaderFooterBlock(block))
                    .sort((a, b) => Number(b.id || 0) - Number(a.id || 0));
                renderBlockCategories(payload.categories || []);
                registerCmsBlocksInGrapes();
                renderCmsBlocks();
                setEditorStatus(`${cmsBlocks.length} bloc${cmsBlocks.length > 1 ? 's' : ''} disponible${cmsBlocks.length > 1 ? 's' : ''}`);
            } catch (error) {
                list.innerHTML = `<div class="hf-block-state">${escapeHtml(error.message || 'Erreur de chargement')}</div>`;
                setEditorStatus('Erreur pendant le chargement des blocs');
            }
        }
