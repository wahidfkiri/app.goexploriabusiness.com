    ...
    <script>
        ...
        async function loadTemplateOnStart(templateId) {
            try {
                ...
                const response = await fetch(`/menus/api/template/${templateId}`);
                ...
                if (data.success && data.data) {
                    const htmlContent = data.data.page_content || '';
                    const cssContent = data.data.page_styles || '';
        ...
        async function saveTemplate() {
            try {
                ...
                const formData = {
                    ...
                    menu_id: window.currentTemplateId
                };
                
                const response = await fetch('/menus/templates/save', {
                    method: 'POST',
                    headers: {
        ...
        function getTemplateIdFromURL() {
            ...
            const pattern1 = /\/menus\/template\/edit\/(\d+)/;
            ...
            const pattern2 = /\/menus\/template\/edit\/(\d+)/;
        ...
        async function loadBlocksModern(templateId) {
            try {
                ...
                let apiUrl = '/menus/api/blocks/data';
        ...
        async function addBlockToEditor(blockId) {
            try {
                ...
                const response = await fetch('/menus/api/blocks/add-to-editor', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': CSRF_TOKEN
