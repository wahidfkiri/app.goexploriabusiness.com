...
class EditeurController extends Controller
{
    ...
    public function previewTemplate($templateId)
    {
        ...
        return view('editor::preview', compact('template'));
    }
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'template_id' => 'nullable|exists:pages,id',
        ...
        try {
            $template = Page::find($request->template_id);
            if ($template) {
                // Mettre à jour le template existant
                $template->update([
                    'html_content' => $request->html_content,
                    'css_content' => $request->css_content ?? '',
                    'thumbnail' => $this->generateThumbnail($request->html_content),
                ]);
            } 
