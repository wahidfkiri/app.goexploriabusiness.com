@php
    ...
    $logoUrl = data_get($quote->metadata, 'logo_url') ?: ($billingSettings->billing_logo_url ?? null);
    $companyName = data_get($quote->metadata, 'etablissement_name') ?: config('app.name');
    ...
    $clientName = data_get($quote->metadata, 'requester_company') ?: data_get($quote->metadata, 'requester_name') ?: $quote->client?->name ?: $quote->client?->lname ?: 'Client';
    $clientAddress = collect([
        data_get($quote->metadata, 'requester_address') ?: $quote->client?->adresse,
        data_get($quote->metadata, 'requester_city') ?: trim(($quote->client?->zip_code ?? '') . ' ' . (($quote->client?->villeRelation?->name ?? $quote->client?->ville) ?? '')),
        data_get($quote->metadata, 'requester_country') ?: $quote->client?->country?->name,
        data_get($quote->metadata, 'requester_email'),
        data_get($quote->metadata, 'requester_phone'),
    ])->filter()->map(fn ($line) => e($line))->implode('<br>');
    $taxBreakdown = collect($quote->taxes_breakdown ?? [])->filter(fn ($tax) => (float) data_get($tax, 'amount', 0) > 0);
    $reference = data_get($quote->metadata, 'reference') ?: data_get($quote->metadata, 'billing_request_number');
@endphp
<!doctype html>
<html lang="fr">
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .top-band { height: 8px; background: #2563eb; border-radius: 8px 8px 0 0; }
        ...
        .doc-type { display: inline-block; padding: 7px 12px; background: #2563eb; color: #ffffff; border-radius: 999px; font-size: 11px; font-weight: 800; text-transform: uppercase; letter-spacing: .08em; }
        ...
        .totals .grand td { background: #2563eb; color: #ffffff; font-size: 16px; font-weight: 800; padding: 13px; }
    ...
    <div class="page">
        ...
        <table class="header">
            <tr>
                ...
                <td style="width:48%;" class="doc-title">
                    ...
                    <div class="doc-status">{{ $quote->status_label ?? ucfirst((string) $quote->status) }}</div>
                </td>
            </tr>
        </table>
            ...
            <tr>
                ...
                <td class="card" style="width:46%;">
                    ...
                    <table class="meta-table">
                        <tr><td class="meta-label">Date devis</td><td class="meta-value">{{ optional($quote->quote_date)->format('d/m/Y') ?: '-' }}</td></tr>
                        <tr><td class="meta-label">{{ $billingSettings->quote_validity_label ?: 'Valide jusqu au' }}</td><td class="meta-value">{{ optional($quote->valid_until)->format('d/m/Y') ?: '-' }}</td></tr>
                        <tr><td class="meta-label">Montant</td><td class="meta-value">{{ $money($quote->total) }}</td></tr>
                        @if($reference)
        ...
        <table class="lines">
            ...
            <tbody>
                ...
                @empty
                    <tr><td colspan="5" class="muted" style="text-align:center;">Aucune ligne de devis.</td></tr>
                @endforelse
            </tbody>
        </table>
...
@endif
                ...
                <td class="totals">
                    <table>
                        <tr><td class="label">Sous-total</td><td class="value">{{ $money($quote->subtotal) }}</td></tr>
                        @if((float) $quote->discount_amount > 0)
                            <tr><td class="label">Remise</td><td class="value">-{{ $money($quote->discount_amount) }}</td></tr>
                        @endif
                        <tr><td class="label">Taxes</td><td class="value">{{ $money($quote->tax_total) }}</td></tr>
                        @if(((float) $quote->shipping_fees + (float) $quote->administration_fees) > 0)
                            <tr><td class="label">Frais</td><td class="value">{{ $money((float) $quote->shipping_fees + (float) $quote->administration_fees) }}</td></tr>
                        @endif
                        <tr class="grand"><td>Total</td><td style="text-align:right;">{{ $money($quote->total) }}</td></tr>
                    </table>
                </td>
            </tr>
        </table>
    ...
    <div class="footer">
        ...
        @if($billingSettings->quote_footer_note)
            - {{ $billingSettings->quote_footer_note }}
        @endif
    </div>
</body>
</html>
