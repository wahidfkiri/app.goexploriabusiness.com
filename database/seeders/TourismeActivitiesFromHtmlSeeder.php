...
class TourismeActivitiesFromHtmlSeeder extends Seeder
{
    /**
     * Parse the tourism category menu HTML and create activities by category.
     */
    public function run(): void
    {
        $htmlPath = database_path('seeders/data/tourisme_activities_menu.html');

        if (!is_file($htmlPath)) {
            $this->command?->error('Fichier introuvable: ' . $htmlPath);
            $this->command?->line('Ajoutez le HTML du menu tourisme dans ce fichier puis relancez le seeder.');
            return;
        }
        ...
        $tourismeType->update(['name' => 'Tourisme', 'is_active' => true]);
        ...
        foreach ($nodesByCategory as $categoryName => $activities) {
            $category = Category::firstOrCreate(
                ...
                [
                    'name' => $categoryName,
                    'categorie_type_id' => $tourismeType->id,
                    'description' => null,
                    'is_active' => true,
                ]
            );

            if ((int) $category->categorie_type_id !== (int) $tourismeType->id) {
        ...
        $this->command?->info("Tourisme activities seed termine. Categories: {$categoriesCount}, creees: {$created}, mises a jour: {$updated}, ignorees: {$skipped}");
    }
